...
public class ListCommandFilter : ICommand {
    ...
    public async Task Execute(string filePath, JsonUtilities jsonUtilities)
    {
        ...
        switch (StatusFilter)
        {
            case "done":        await jsonUtilities.ListElementInArrayAsync(filePath, "Status", TodoTaskStatus.DONE);        break;
            case "todo":        await jsonUtilities.ListElementInArrayAsync(filePath, "Status", TodoTaskStatus.TODO);        break;
            case "in-progress": await jsonUtilities.ListElementInArrayAsync(filePath, "Status", TodoTaskStatus.IN_PROGRESS); break;
